...
class Artist_searcher:
    ...
    def get_pymongo(self):
        if self.select_type == "name":
            return collection.find({"name":self.query})

        if self.select_type == "alias":
            return collection.find({"aliases.name":self.query})

        if self.select_type == "area":
            return collection.find({"area":self.query})

        if self.select_type == "tag":
            return collection.find({"tags.value":self.query})
